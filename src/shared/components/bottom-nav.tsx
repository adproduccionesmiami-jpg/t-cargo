...
import { Home, Truck, Search, Package } from 'lucide-react'
...
const navItems = [
    ...
    { label: 'PAQUETES', icon: Package, href: '/deliveries' },
]
...
function BottomNavContent() {
    ...
    const isItemActive = (href: string) => {
        ...
        if (href === '/deliveries') {
            return pathname.startsWith('/deliveries')
        }
        ...
        <nav className="fixed bottom-0 left-0 right-0 z-50">
            <div className="max-w-md mx-auto bg-white border-t border-gray-100 px-6 py-3 flex justify-between items-center rounded-t-3xl shadow-[0_-4px_20px_rgba(0,0,0,0.06)]">
                {navItems.map((item) => {
                    ...
                    // Si es paquetería, usamos color esmeralda; de lo contrario, ámbar.
                    const activeColor = item.href === '/deliveries' ? 'text-emerald-500' : 'text-[#f59e0b]'

                    return (
                        <Link
                            key={item.label}
                            href={item.href}
                            className={`flex flex-col items-center gap-1 transition-colors ${isActive ? activeColor : 'text-gray-400'
                                }`}
                        >
